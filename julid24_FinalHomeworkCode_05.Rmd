...
reg_boot <- function(data, idx) {
  ...
  coef(fit)
} #this is the formula the bootstrap will follow in order to fit the same model. It must include a data frame to sample from and an indices parameter (idx). 

b <- boot(data, reg_boot, 1000) #the boot() function generates a set of random indices with replacement R times. The bootstrap samples and results will be held in 'b' or the bootobject. 
b_coefs <- b$t #t represents a R x k matrix where each row is a bootstrap replicate 
```
